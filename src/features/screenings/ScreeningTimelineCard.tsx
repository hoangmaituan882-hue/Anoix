  ...
  return (
    <div className="relative flex items-start gap-4 sm:gap-6 group">
      ...
      <div className="flex flex-col items-center shrink-0 pt-3 z-10">
        {/* Crisp Studio Dot */}
        <div className="w-8 h-8 rounded-full bg-white border-2 border-[#ff3650] flex items-center justify-center group-hover:bg-[#ff3650] transition-colors duration-200">
          <span className="w-2.5 h-2.5 rounded-full bg-[#ff3650] group-hover:bg-white transition-colors duration-200" />
        </div>

        {/* Year indicator */}
        <span className="mt-1.5 text-[11px] font-mono font-bold text-black/50 group-hover:text-white transition-colors">
          {year}
        </span>
      </div>
      ...
      <motion.article
        ...
        className="flex-1 bg-white border border-black/10 hover:border-[#ff3650]/60 rounded-2xl p-4 sm:p-6 transition-colors duration-200"
      >
        <div className="grid grid-cols-1 md:grid-cols-12 gap-5 items-start">
          ...
          <div className="md:col-span-4 lg:col-span-3 flex flex-col items-center">
            <div 
              ...
              className="relative w-full aspect-[27/38] rounded-xl overflow-hidden border border-black/10 cursor-pointer group/poster bg-black/60"
            >
              <img
                src={poster}
                alt={title}
              ...
              {/* Date Chip */}
              <div className="absolute top-2 left-2 bg-black/85 px-2 py-0.5 rounded text-[10px] font-mono font-bold text-white border border-black/10">
            ...
            <button
              onClick={() => onOpenPoster(screening)}
              className="mt-2 text-[11px] font-bold text-black/50 hover:text-[#ff3650] flex items-center gap-1 transition-colors cursor-pointer"
            >
          ...
          <div className="md:col-span-8 lg:col-span-9 flex flex-col justify-between h-full">
            <div>
              ...
              <div className="flex flex-wrap items-center gap-2 mb-2">
                ...
                {screening.time && (
                  <span className="inline-flex items-center gap-1 text-[11px] font-mono text-black/70 bg-white/5 px-2.5 py-0.5 rounded border border-black/10">
                ...
                {screening.format_tags?.map((tag, idx) => (
                  <span
                    key={idx}
                    className="text-[11px] font-medium text-black/80 bg-white/5 px-2 py-0.5 rounded border border-black/10"
                  >
                    {tag}
                  </span>
                ))}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-3 text-xs bg-white/[0.02] p-3 rounded-xl border border-black/5">
                {/* Special Guests */}
                {screening.special_guests && screening.special_guests.length > 0 && (
                  <div>
                    <span className="text-[10px] font-bold text-black/40 uppercase tracking-wider block mb-1">
              ...
              {/* Recap Summary */}
              {recapText && (
                <p className="text-xs text-black/70 leading-relaxed mb-4 pl-3 border-l-2 border-[#ff3650]/50">
                  {recapText}
                </p>
              )}
            </div>
                ...
                {screening.film_ids?.map((fid) => {
                  ...
                  return (
                    <button
                      ...
                      className="text-[11px] font-medium text-black/80 bg-white/5 hover:bg-[#ff3650] hover:text-white px-2.5 py-0.5 rounded transition-colors cursor-pointer border border-black/10"
                    >
                      {filmTitle}
                    </button>
                  );
