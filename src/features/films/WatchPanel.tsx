  ...
  return (
    <div className="bg-[#1a1a1a] border border-white/10 rounded-2xl p-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs font-bold text-white/40">{rating > 0 ? `${rating}/5` : '未评分'}</span>
      </div>
      ...
      <textarea
        ...
        className="mt-3 w-full bg-black/50 border border-white/15 rounded-xl px-3.5 py-2.5 text-white text-sm font-medium focus:border-[#ff3650] focus:ring-1 focus:ring-[#ff3650] focus:outline-none transition-all placeholder:text-white/30 resize-none"
      />
      <p className="text-[11px] text-white/30 text-right mt-1">{review.length}/200</p>

      <div className="flex items-center justify-between mt-3">
        <button
          onClick={remove}
          className="inline-flex items-center gap-1.5 text-xs font-bold text-white/40 hover:text-[#ff3650] transition-colors cursor-pointer"
        >
