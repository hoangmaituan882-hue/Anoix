...
const avatarCls = (u: AdminUser) =>
  u.role === 'admin'
    ...
    : 'bg-white/10 text-black/60 border-black/15';
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-black/10 shadow-xl">
        <div className="space-y-1">
          ...
          <p className="text-xs text-black/50">管理登录账号、管理员角色与登录封禁（封禁即禁止登录）</p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-4 rounded-2xl border border-black/10">
        <div className="relative flex-1 sm:max-w-xs">
          ...
          <input
            ...
            className="w-full bg-black/40 border border-black/15 rounded-xl pl-9 pr-4 py-2 text-xs text-white placeholder:text-black/40 focus:border-[#ff3650] focus:outline-none"
          />
        </div>
            ...
            {(['all', 'user', 'admin'] as const).map((r) => (
              <button
                ...
                className={`px-3 py-1 rounded-lg text-xs font-black transition-colors cursor-pointer ${
                  roleFilter === r ? 'bg-[#ff3650] text-white' : 'text-black/60 hover:text-white'
                }`}
              >
          ...
          <div className="flex items-center bg-black/40 border border-black/15 rounded-xl p-0.5">
            {(['all', 'active', 'disabled'] as const).map((s) => (
              <button
                ...
                className={`px-3 py-1 rounded-lg text-xs font-black transition-colors cursor-pointer ${
                  statusFilter === s ? 'bg-[#e0fe3d] text-[#121212]' : 'text-black/60 hover:text-white'
                }`}
              >
      ...
      {filtered.length === 0 ? (
        <div className="bg-white border border-black/10 rounded-3xl p-16 text-center space-y-3">
          <User className="w-12 h-12 text-black/20 mx-auto" />
          <p className="text-base font-bold text-black/70">未找到匹配的用户</p>
          <p className="text-xs text-black/40">可尝试重置筛选条件或新建账号</p>
        </div>
      ) : (
        <div className="bg-white border border-black/10 rounded-2xl overflow-hidden shadow-xl divide-y divide-white/5">
          {filtered.map((u) => (
            <div key={u.uid} className="p-4 sm:p-5 flex flex-col lg:flex-row lg:items-center gap-4 hover:bg-white/[0.03] transition-colors">
              ...
              <div className="flex items-center gap-3.5 min-w-0 flex-1">
                ...
                <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-2 text-xs text-black/40 font-mono mt-1 flex-wrap">
                    {u.username && <span>@{u.username}</span>}
                    {u.email && (
                      <span className="inline-flex items-center gap-1"><Mail className="w-3 h-3" />{u.email}</span>
                    )}
                    <span className="text-black/25">{u.uid}</span>
                  </div>
                </div>
              </div>

              {/* Meta */}
              <div className="hidden xl:flex items-center gap-6 text-xs text-black/40 shrink-0">
                <div className="text-right">
                  <span className="text-[10px] text-black/30 uppercase block">注册时间</span>
                  <span className="font-mono">{u.registeredAt ? u.registeredAt.slice(0, 10) : (u.createTime || '—')}</span>
                </div>
                <div className="text-right">
                  <span className="text-[10px] text-black/30 uppercase block">密码</span>
                  <span className={`font-mono ${u.hasPassword ? 'text-emerald-400' : 'text-black/30'}`}>{u.hasPassword ? '已设置' : '未设置'}</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-1.5 shrink-0 flex-wrap">
                <button
                  ...
                  disabled={busy}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-black transition-colors cursor-pointer disabled:opacity-40 border border-black/10 bg-white/5 hover:bg-white/15 text-white"
                ...
                <button
                  onClick={() => remove(u)}
                  disabled={busy}
                  className="p-1.5 rounded-lg text-black/30 hover:text-[#ff3650] hover:bg-white/5 transition-colors cursor-pointer disabled:opacity-40"
                  title="删除账号"
                >
      ...
      {createOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-xl animate-fade-in" onClick={() => setCreateOpen(false)}>
          <div className="w-full max-w-md bg-white border border-black/20 rounded-3xl p-6 sm:p-8 space-y-5 text-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-black/10 pb-3">
              ...
              <button onClick={() => setCreateOpen(false)} className="w-9 h-9 rounded-full bg-white/10 hover:bg-[#ff3650] text-white flex items-center justify-center transition-colors cursor-pointer border border-black/10">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <Lock className="w-4 h-4 absolute right-3.5 top-1/2 -translate-y-1/2 text-black/30" />
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between border-b border-black/10 pb-3">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-black/40">{resetTarget.nickname || resetTarget.username}</p>
                </div>
              </div>
              <button onClick={() => setResetTarget(null)} className="w-9 h-9 rounded-full bg-white/10 hover:bg-[#ff3650] text-white flex items-center justify-center transition-colors cursor-pointer border border-black/10">
                <X className="w-4 h-4" />
              </button>
            </div>
