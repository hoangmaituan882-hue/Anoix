  ...
  return (
    <div className="space-y-5">
      ...
      {goods === null ? (
        ...
        <p className="text-white/40 text-sm py-6">暂无商品。</p>
      ) : (
        <div className="space-y-2.5">
          {goods.map((g) => (
            <div key={g.id} className="flex items-center gap-3 rounded-xl border border-white/10 bg-[#1a1a1a] px-3 py-2.5">
              {g.image ? <img src={g.image} alt="" className="w-12 h-16 rounded-md object-cover shrink-0 bg-black/40" /> : <div className="w-12 h-16 rounded-md bg-white/5 shrink-0 flex items-center justify-center text-white/20 text-[10px]">无图</div>}
              <div className="min-w-0 flex-1">
                <p className="text-sm font-bold text-white truncate">{g.title}</p>
                <p className="text-[11px] text-white/40 truncate">{g.series || '—'}</p>
                <div className="flex items-center gap-2 mt-1">
                  ...
                  ) : <span className="text-[10px] font-bold text-white/30">无淘宝链接</span>}
                </div>
              </div>
              <div className="flex items-center gap-1.5 shrink-0">
                <button onClick={() => setEditing({ ...g })} className="w-8 h-8 rounded-lg bg-white/5 hover:bg-white/15 text-white/60 hover:text-white inline-flex items-center justify-center transition-colors cursor-pointer" title="编辑"><Pencil className="w-4 h-4" /></button>
                <button onClick={() => remove(g)} className="w-8 h-8 rounded-lg bg-white/5 hover:bg-[#ff3650] text-white/60 hover:text-white inline-flex items-center justify-center transition-colors cursor-pointer" title="删除"><Trash2 className="w-4 h-4" /></button>
              </div>
            </div>
          ))}
        </div>
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-xl" onClick={() => setEditing(null)}>
          <div className="w-full max-w-xl max-h-[88vh] overflow-y-auto bg-[#181818] border border-white/15 rounded-3xl p-6 text-white" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3.5">
              ...
              <div className={FIELD}>
                ...
                <p className="text-[11px] text-white/30">购买按钮将跳转到此链接（新标签打开）</p>
              </div>
              <div className={FIELD}>
                <Label className={LABEL}>描述</Label>
                <textarea value={editing.description ?? ''} onChange={(e) => setEditing({ ...editing, description: e.target.value })} rows={2} className="w-full bg-black/50 border border-white/15 rounded-xl px-3.5 py-2.5 text-white text-sm focus:border-[#ff3650] focus:ring-1 focus:ring-[#ff3650] focus:outline-none resize-none" placeholder="商品描述" />
              </div>
              <label className="flex items-center gap-2.5 cursor-pointer">
                ...
                <span className="text-sm font-bold text-white/70">预售商品</span>
              </label>
            </div>
