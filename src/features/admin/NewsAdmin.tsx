...
const FIELD = 'w-full bg-black/50 border border-black/15 rounded-xl px-3.5 py-2.5 text-white text-sm font-medium focus:border-[#4246ff] focus:ring-1 focus:ring-[#4246ff] focus:outline-none transition-all placeholder:text-black/30';
...
const STATUS_META: Record<string, { label: string; cls: string }> = {
  draft: { label: '草稿', cls: 'bg-white/10 text-black/50 border-black/20' },
  ...
  archived: { label: '已归档', cls: 'bg-white/5 text-black/30 border-black/10' },
};
...
export const NewsAdmin: React.FC<{ onCountChange?: (count: number) => void }> = ({ onCountChange }) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-black/10 shadow-xl">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <span className="bg-white/5 text-black/50 px-2 py-0.5 rounded-full text-xs font-mono">
              {rows.length} 篇库存
            </span>
          </div>
      ...
      <div className="relative w-full py-10 md:py-14 px-4 sm:px-8 overflow-hidden select-none bg-[#4246ff] rounded-3xl border border-black/10">
        ...
        <div className="relative z-10 max-w-5xl mx-auto">
          <div className="mb-8 flex items-end justify-between gap-3">
            ...
            <span className="text-[10px] font-mono text-black/70 hidden sm:inline">拖动手柄排序 · 点击编辑</span>
          </div>

          {previewItems.length > 0 && (
            <div className="mb-8 border-y border-black/15 py-3 overflow-hidden">
          ...
          <div className="news_list divide-y divide-white/20 border-t border-b border-black/20">
            {previewItems.length === 0 ? (
              <p className="py-12 text-center text-black/80 font-bold">暂无已发布动态。首页这一块仍会显示，访客会看到空列表。</p>
            ) : (
              previewItems.map((item, index) => {
                ...
                return (
                  ...
                  >
                    <span className="text-black/50 shrink-0" title="拖动排序">
                    ...
                    {item.image ? (
                      <img
                        src={item.image}
                        alt=""
                        className="w-12 h-12 sm:w-16 sm:h-16 rounded-xl object-cover shrink-0 border border-black/10 bg-black/40"
                      />
                    ) : null}
                    <button
                      type="button"
                    ...
                    >
                      <div className="flex items-center gap-3 text-sm font-bold text-black/80 flex-wrap">
                      ...
                      <h4 className="mt-1 text-lg sm:text-xl font-bold text-white group-hover:text-[#1e1f21] line-clamp-2">
                        {displayTitle(item)}
                      </h4>
                    </button>
                    <button
                      type="button"
                      onClick={() => row && setEditing(fromRow(row))}
                      className="hidden md:inline-flex items-center text-black/80 hover:text-white cursor-pointer"
                      title="编辑"
                    >
          ...
          <div className="mt-8 flex justify-start pointer-events-none">
            <span className="design_button inline-flex items-center gap-3 bg-[#f5ffe5] text-[#121212] px-7 py-3 rounded-full font-black text-sm tracking-wider uppercase shadow-xl">
              查看全部
              <span className="w-7 h-7 rounded-full bg-[#f5ffe5] text-[#1e1f21] flex items-center justify-center">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-3 rounded-2xl border border-black/10">
        ...
        <div className="relative sm:w-64 shrink-0">
          ...
          <input
            ...
            placeholder="搜索公告..."
            className="w-full bg-white/5 border border-black/15 rounded-full pl-9 pr-3 py-1.5 text-xs text-white placeholder:text-black/40 focus:outline-none focus:border-[#4246ff] transition-colors"
          />
        </div>
      </div>

      {filteredNews.length === 0 ? (
        <div className="py-16 text-center text-black/40 font-bold">暂无匹配的动态</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredNews.map((r) => (
            <div
              key={r.id}
              className="group bg-white border border-black/10 rounded-2xl p-5 hover:border-[#4246ff]/50 hover:bg-[#1e1e1e] transition-colors shadow-sm"
            >
              <div className="flex items-center gap-2 mb-2 flex-wrap">
                ...
                <span className="text-xs font-mono text-black/50 ml-auto">{r.date}</span>
              </div>
              <h4 className="font-black text-white text-base line-clamp-1 mb-1 group-hover:text-[#8b8eff] transition-colors">
                {r.title_zh || r.title}
              </h4>
              {r.title_zh && <p className="text-xs text-black/40 font-mono line-clamp-1 mb-1">{r.title}</p>}
              {(r.content_zh || r.content) && (
                <p className="text-xs text-black/60 line-clamp-2">{r.content_zh || r.content}</p>
              )}
              <div className="mt-3 flex flex-wrap items-center justify-end gap-2">
                ...
                {r.status === 'published' && (
                  <button type="button" onClick={() => setStatus(r.id, 'archived')} className="px-2.5 py-1 rounded-lg bg-white/10 hover:bg-white/20 text-xs font-bold text-black/70 cursor-pointer">归档</button>
                )}
                {r.status === 'archived' && (
                ...
                <button
                  ...
                  className="px-2.5 py-1 rounded-lg bg-white/5 hover:bg-white/15 text-xs font-bold text-black/70 cursor-pointer inline-flex items-center gap-1 border border-black/10"
                >
                ...
                <button
                  type="button"
                  onClick={() => remove(r.id)}
                  className="px-2.5 py-1 rounded-lg bg-white/5 hover:bg-[#ff3650] text-xs font-bold text-black/50 hover:text-white transition-colors cursor-pointer inline-flex items-center gap-1 border border-black/10"
                >
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-xl" onClick={() => setEditing(null)}>
          <div
            className="w-full max-w-2xl max-h-[92vh] overflow-y-auto bg-white border border-black/20 rounded-3xl p-6 sm:p-8 space-y-4 text-white"
          ...
          >
            ...
            <div className="rounded-2xl bg-[#4246ff] px-4 py-3">
              <p className="text-[10px] font-black text-black/70 uppercase tracking-widest">首页预览标题</p>
            ...
            <div className="flex items-center justify-between gap-3 flex-wrap pt-1">
              <span className="inline-flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-black/60">{editing.pinned ? '已置顶（首页最前）' : '置顶到首页最前'}</span>
              </span>
            </div>
            ...
            <div className="flex flex-wrap justify-end gap-2 pt-2">
              ...
              <button type="button" onClick={() => void saveDraft('draft')} disabled={busy} className="px-4 py-2 rounded-xl border border-black/15 text-xs font-bold text-black/70 cursor-pointer disabled:opacity-50">
                存为草稿
              </button>
              <button
                type="button"
