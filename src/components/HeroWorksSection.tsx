...
import { HERO_IMAGE, WORKS_LIST, I18N } from '../data/triggerData';
import { ArrowRight, ChevronLeft, ChevronRight, Play } from 'lucide-react';

interface HeroWorksSectionProps {
  lang: Language;
  onSelectWork: (work: WorkItem) => void;
  onOpenAllWorks: () => void;
}

export const HeroWorksSection: React.FC<HeroWorksSectionProps> = ({
  lang,
  onSelectWork,
  onOpenAllWorks,
}) => {
  ...
  return (
    ...
    >
      {/* LEFT COLUMN: Red Character Visual */}
      <div
      ...
      >
        <picture className="w-full h-full flex items-center justify-center">
          <source media="(max-width: 450px)" srcSet={HERO_IMAGE} />
          <img
            fetchPriority="high"
            src={HERO_IMAGE}
            ...
            >
              {WORKS_LIST.map((work) => (
                <article
                  ...
                  className="flex-shrink-0 w-[170px] sm:w-[210px] md:w-[240px] group/card rounded-2xl overflow-hidden cursor-pointer transition-all duration-300 transform hover:-translate-y-2 hover:shadow-[0_12px_30px_rgba(255,54,80,0.35)]"
                >
                  <div className="relative aspect-[27/40] rounded-2xl overflow-hidden bg-black/40 border-2 border-white/10 group-hover/card:border-[#ff3650] transition-colors">
                    <img
                      src={work.image}
