...
export interface SelectContentProps {
  className?: string;
  children: ReactNode;
  /** Max height (px) of the scrollable option list; enables inner scrolling. */
  maxHeight?: number;
}

export function SelectContent({ className, children, maxHeight }: SelectContentProps) {
  ...
  return (
    ...
    >
      <motion.div
        ...
        className={cn("p-1", maxHeight ? "overflow-y-auto" : undefined)}
...
export interface SelectItemProps {
  ...
  className?: string;
  children: ReactNode;
  /** Display string for the closed trigger when children are non-string nodes. */
  label?: string;
  /** React 19 lists pass `key` through to the component prop type. */
  key?: string | number;
}
...
export function SelectItem({
  ...
  useLayoutEffect(() => {
    ctx.register(value, resolvedLabel);
    return () => ctx.unregister(value);
  }, [ctx.register, ctx.unregister, value, resolvedLabel]);
