...
| POST | /api/vote | 必选 | vote 30/min | `{ filmId, count? }` 周叠票（默认 +1，封顶剩余配额） |
| DELETE | /api/vote | 必选 | vote 30/min | `{ filmId }` 本周该片 count −1 |
| GET | /api/quota | 可选 | 无 | 周配额（匿名给默认额度） |
| POST | /api/nominations | 必选 | nom 20/min | 持续提名 → 提名池；同人同片同周一票 409 `already_nominated_this_week` |
...
撤票：本周该片 count −1；减到 0 则删行；`unbumpQuota(..., 1)`。无 `filmId` → 400。

## 错误码

| 码 | 值 |
|---|---|
| 400 | `bad_request` / `note_required` / `note_too_long` / `film_required` |
| 401 | `identity_required` |
| 404 | `film_not_found` / `vote_not_found` |
| 409 | `already_screened` / `frozen` / `already_nominated_this_week` |
...
- 不再提供轮次票、`GET /api/nominations` 轮次列表、`POST /api/nominations/:roundId/nominate`。
